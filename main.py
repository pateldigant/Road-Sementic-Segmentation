...
def layers(vgg_layer3_out, vgg_layer4_out, vgg_layer7_out, num_classes):
    ...
    up_conv_1x1_l7 = tf.layers.conv2d_transpose(conv_1x1_l7,num_classes,4,2,padding='same',
                                kernel_regularizer= tf.contrib.layers.l2_regularizer(1e-3))
    #add upsampled l7 to l4 1x1
    sum1 = tf.add(up_conv_1x1_l7,conv_1x1_l4)
    #upsample the sum
    sum1_up = tf.layers.conv2d_transpose(sum1,num_classes,4,2,padding='same',
                                kernel_regularizer= tf.contrib.layers.l2_regularizer(1e-3))
    #add to l3
    sum2 = tf.add(sum1_up,conv_1x1_l3)
    #upsample it 8 times
    final = tf.layers.conv2d_transpose(sum2,num_classes,16,8,padding='same',
                                kernel_regularizer= tf.contrib.layers.l2_regularizer(1e-3))
    return final
...
def train_nn(sess, epochs, batch_size, get_batches_fn, train_op, cross_entropy_loss, input_image,
    ...
    for epoch in range(epochs):
        ...
        for image, label in get_batches_fn(batch_size):
            ...
            all_loss.append('{:3f}'.format(loss))
        print(all_loss)
        print()
